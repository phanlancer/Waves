...
class Matcher(actorSystem: ActorSystem,
  ...
  private val matcherExtension                                                         = new MatcherExtension(matcherSettings, blockchain)
  ...
  lazy val matcherApiRoutes = Seq(
    MatcherApiRoute(wallet, matcherExtension, matcher, orderHistory, ap => Option(orderBookCache.get(ap)), txWriter, restAPISettings, matcherSettings)
  )
